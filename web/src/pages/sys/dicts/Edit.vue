<template>
  <el-form ref="form" :model="data" :rules="rules">
    ...
    <el-form-item :label="$t('l.Type')" prop="type">
      <el-input v-model="data.type" clearable maxlength="100" show-word-limit></el-input>
    </el-form-item>
    <el-form-item :label="$t('l.Code')" prop="code">
...
const { form, data, rules, save } = useEdit({
  ...
  rules: {
    type: {
      required: true,
      message: t("r.Input").replace("$v", t("l.Type").toLowerCase()),
      trigger: "blur",
    },
    code: {
      required: true,
      message: t("r.Input").replace("$v", t("l.Code").toLowerCase()),
      trigger: "blur",
    },
    name: {
      required: true,
