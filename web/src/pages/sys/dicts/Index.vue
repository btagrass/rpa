<template>
  <div>
    ...
    <el-table ref="table" :data="data.records" border @selection-change="select">
      ...
      <el-table-column :label="$t('l.Type')" prop="type"></el-table-column>
      <el-table-column :label="$t('l.Code')" prop="code"></el-table-column>
      <el-table-column :label="$t('l.Name')" prop="name"></el-table-column>
      <el-table-column :label="$t('l.Sequence')" prop="sequence"></el-table-column>
...
<script setup name="dicts">
import { useComp, useList } from "@/crud"
import Edit from "./Edit.vue"

const { name, values, visible, open, close } = useComp()
const { table, query, data, list, remove, select } = useList("/mgt/sys/dicts")
</script>
